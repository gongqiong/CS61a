...
def balanced(m):
    ...
    if is_weight(end(left(m))) and is_weight(end(right(m))):
        if length(left(m)) * size(end(left(m))) == length(right(m)) * size(end(right(m))):
            return True
        else:
            return False
    else:
...
def make_fib():
    ...
    def nextfib():
        nonlocal fiblist
        result = fiblist[0]
        fiblist = [fiblist[1], fiblist[0] + fiblist[1]]
        return result
...
# 关于nonlocal，dictionary插入不是赋值操作，而是方法调用（method call）
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, p):
        ...
        if len(plist) < 3:
            ...
            return "Your account is locked. Attempts: " + str(plist)
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(test) == str:
        ...
        correctP=[old_password,new_password]
        def change(amount,password):
            nonlocal correctP
            if password in correctP:
