...
def merge(lst1, lst2):
    ...
    """
    "*** YOUR CODE HERE ***"
    # 不排序 mergelist=lst1+[i if i not in lst1 else [] for i in lst2]
    # recursive
...
def print_board(board, max_rows, max_cols):
    ...
    for row in range(max_rows):
        ...
        for col in range(max_cols):
            col_str = get_piece(board, row, col)
            row_str += col_str + ' '
        print(row_str.strip())
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    diagonal_win = check_win_diagonal(board, max_rows, max_cols, num_connect,
                                      row, col, player)
    "*** YOUR CODE HERE ***"
    if check_win_row(board,max_rows,max_cols,num_connect,row,player):
        return True
    elif check_win_column(board,max_rows,max_cols,num_connect,col,player):
        return True
    elif diagonal_win:
        return True
    else:
